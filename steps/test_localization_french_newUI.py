...
def launch_browser(browser):
    global LOGIN_PAGE, DASHBOARD_PAGE, DRIVER_PROFILE_PAGE, ASSIGN_DRIVER_PAGE, COACHING_PAGE, EVENT_PAGE, LDI, LIBRARY_PAGE
...
def verify_login():
    ...
    assert_that(DASHBOARD_PAGE.get_tasks_new_UI_title(), IsEqualIgnoringCase(LDI.tasks_fr_label))
    assert_that(DASHBOARD_PAGE.get_metrics_new_UI_title(), IsEqualIgnoringCase(LDI.metrics_fr_label))
...
def verify_overlay_label():
    assert_that(EVENT_PAGE.get_overlay_label_text(), IsEqualIgnoringCase(LDI.overlay_fr_label))
...
def verify_overlay_label():
    assert_that(EVENT_PAGE.get_overlay_toggle_text(), IsEqualIgnoringCase("La superposition n'est pas disponible pour cette vidéo"))
...
def download_mp4_overlay():
    ...
    EVENT_PAGE.click_download_option_mp4_with_overlay()


@then('the file name is displayed in the selected language format')
def verify_file_name_format():
    expected_file_name = f"{LDI.fd_event_with_adas_data}-{LDI.overlay_fr_label}.MP4"
