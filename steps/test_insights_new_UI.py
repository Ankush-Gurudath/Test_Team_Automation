...
coachable_events_count = ""
events_count = ""

scenarios('../features/insights_new_UI.feature')
...
def get_profile_link_behave(context):
    DRIVERS_REPORT_PAGE.click_reset()
    context['continual_driver'] = DRIVERS_REPORT_PAGE.get_first_driver_name()
...
def verify_driver_profile_behave_chosen_result(context):
    assert_that(DRIVERS_REPORT_PAGE.get_driver_profile_behave_text(), contains_string(context['continual_driver']))


@when('the user clicks Continual Behaviors tab in driver profile')
def back_to_continual_behave_tab():
    DRIVERS_REPORT_PAGE.click_continual_behave_tab_in_driver_profile()


@then('Browse or Wake Video Links is displayed in Drivers profile page')
def verify_browse_or_wake_video_links():
    assert DRIVERS_REPORT_PAGE.browse_or_wake_link_is_displayed() is True
...
def coaches_filter_group():
    ...
    COACHES_REPORT_PAGE.search_filter_by_group_coaches(ID.group)
...
def verify_filter_by_group_coach():
    assert_that(COACHES_REPORT_PAGE.get_group_name(), contains_string(ID.group))
...
def type_driver_name_in_searchbox():
    SAFE_DRIVING_REPORT_PAGE.click_reset()
    SAFE_DRIVING_REPORT_PAGE.search_driver_name(ID.driver)
...
def click_first_group_coachable_events_count():
    DASHBOARD_PAGE.click_insights_menu()
    DASHBOARD_PAGE.click_group_report_submenu_new_UI()
    GROUP_REPORT_PAGE.click_reset_alert()
    global coachable_events_count
    coachable_events_count = GROUP_REPORT_PAGE.get_coachable_events_count()
    GROUP_REPORT_PAGE.click_group_coachable_events_count()
    global events_count
    events_count = GROUP_REPORT_PAGE.get_events_count()


@then('the events page events count should be same as Group report coachable events count')
def verify_events_count_matches_coachable_events_count():
    assert events_count == coachable_events_count


#LQ-TB
@when('the user clicks download driver event history report')
def download_driver_history_report(context):
    DASHBOARD_PAGE.click_insights_menu()
    DASHBOARD_PAGE.click_drivers_report_submenu_new_UI()
    DRIVERS_REPORT_PAGE.click_reset_driver_score()
    DRIVERS_REPORT_PAGE.search_by_name(ID.driver2)
    ...
    DRIVERS_REPORT_PAGE.click_driver_scores_link()
    context['file'] = False
    if DRIVERS_REPORT_PAGE.dropdown_is_displayed():
        context['file'] = True
        DRIVERS_REPORT_PAGE.click_download()
        DRIVERS_REPORT_PAGE.click_driver_event_history_report()
    else:
        DRIVERS_REPORT_PAGE.click_download_csv()



@then('the report is downloaded successfully')
def verify_download_driver_event_history_report(context):
    if context['file']:
        file_name = DRIVERS_REPORT_PAGE.get_driver_event_history_file_name(ID.driver2)
    else:
        file_name = DRIVERS_REPORT_PAGE.get_driver_name_file_name(ID.driver2)
    DRIVERS_REPORT_PAGE.wait_for_file_downloaded(file_name)
    assert DRIVERS_REPORT_PAGE.check_file_exist(file_name) is True
