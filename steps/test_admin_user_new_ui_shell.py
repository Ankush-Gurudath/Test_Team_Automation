...
scenarios('../features/admin_user_new_ui_shell.feature')
...
def user_should_see_buttons_import_users_and_add_users():
    # assert USER_MANAGEMENT_PAGE.import_button_is_displayed()
...
def verify_action_type_in_view_history():
    assert_that(USER_MANAGEMENT_PAGE.get_expected_action_type_text("Added"), contains_string("Added"))
...
# LQ-288388
@when('the user is on the first page of users')
def navigate_to_first_page_user_management():
...
def verify_first_page_user_management():
...
def click_next_page_user_management():
    USER_MANAGEMENT_PAGE.click_next_page_button()

@then('the next page of users is displayed and page 2 is selected')
def verify_next_page_user_management():
...
def click_previous_page_user_management():
    USER_MANAGEMENT_PAGE.click_previous_page_button()

@then('the previous page of users is displayed and page 1 is selected')
def verify_previous_page_user_management():
    assert USER_MANAGEMENT_PAGE.get_current_page_number() == 1

@when('the user clicks last page number in the pagination section')
def enter_last_page_number_user_management():
    USER_MANAGEMENT_PAGE.click_last_page_button()

@then('the user list navigates to last page and the Next button in the pagination section is disabled')
def verify_last_page_user_management():
