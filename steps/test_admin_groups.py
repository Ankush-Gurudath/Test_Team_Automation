...
scenarios('../features/admin_groups.feature')
...
def pm_log_in(browser):
    ...
    LOGIN_PAGE.retry_if_login_failed(DC_URL, AD.fa_gm_user_name, AD.password, AD.company_name)
...
@then(parsers.parse('drop down should not present'))
...
@then('The user should be able to see the filters ‘Search Group Affected’, Date filter and ‘Select Action(s)’ in the header, and the ‘Download Log’ button')
...
def verify_download_log_group_audit_log():
    ...
    assert ADMIN_GROUPS_PAGE.check_file_exist(file_name) is True
