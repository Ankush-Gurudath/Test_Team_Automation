...
from steps.common import FLEET_URL, ENV, NEW_UI_FTM_URL
...
scenarios('../features/fleet_maint_new_ui_shell.feature')
...
@when('the user clicks the dropdown of "Maintenance" menu')
def go_to_fleet_maint_pm():
    FLEET_MAINT_PAGE.click_maintenance_new_ui()


@then('the "Maintenance" menu should expand to show the  "Preventative Maintenance" and "Diagnostic Trouble Codes"')
def assert_maintenance_menu_expanded():
    assert FLEET_MAINT_PAGE.preventative_maintenance_menu_text() == "Preventative Maintenance"
    assert FLEET_MAINT_PAGE.diagnostic_trouble_codes_menu_text() == "Diagnostic Trouble Codes"
...
@then('the DTC page is displayed and the number of Codes, group filter and reset icon are displayed on the header bar '
      'and the table is displayed with columns: "VEHICLE", "GROUP", "DATE", "CODE (DESCRIPTION)"')
def assert_dtc():
...
def filter_group_dtc_page():
    ...
    FLEET_MAINT_PAGE.search_by_group_dtc_page(FMD.dtc_group)
...
def assert_group_filter_dtc():
    assert FLEET_MAINT_PAGE.get_row_count() == FMD.dtc_code_count
