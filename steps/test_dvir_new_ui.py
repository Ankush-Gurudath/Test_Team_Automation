...
scenarios('../features/DVIR_new_ui.feature')
...
def refresh_browser():
    DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.click_refresh_button()
    DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.wait_for_page_load()


@then('the user should see the left navigation with the modules')
def verify_dvir_page_loaded():
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.dvir_tab_new_ui_is_displayed() is True
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.schedules_tab_new_ui_is_displayed() is True
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.list_settings_tab_new_ui_is_displayed() is True


@when('the user navigates to driver safety and clicks the Back button from Browser')
def navigate_back():
...
@then('the user be navigated to "DVIR" application again')
def verify_navigated_back_to_dvir_page():
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.dvir_tab_new_ui_is_displayed() is True


@then('the user should see the new logo "lytx+ GEOTAB"')
def verify_lytx_geotab_logo():
    assert DASHBOARD_PAGE.lytx_geotab_logo_is_displayed() is True


@then('the user should see the modules such as "DVIR","Schedules" and "List Settings" present in the left panel')
def verify_dvir_schedules_list_settings_tabs():
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.dvir_tab_new_ui_is_displayed() is True
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.schedules_tab_new_ui_is_displayed() is True
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.list_settings_tab_new_ui_is_displayed() is True


@when('the user clicks the dropdown for List Settings')
def click_list_settings_dropdown():
    DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.click_list_settings_existing_user_tab()


@then('the user should see two sub-menus named "List Management" and "List Assignment"')
def verify_list_settings_sub_menus():
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.list_management_tab_existing_user_is_displayed() is True
    assert DRIVER_VEHICLE_INSPECTION_REPORTS_PAGE.assignment_tab_existing_user_is_displayed() is True
...
def verify_inspection_management_titles():
    ...
    assert INSPECTION_LIST_MANAGEMENT_PAGE.list_management_highlighted() is True
...
@when('the user clicks on the "New List" button in Vehicle Inspection List page')
def click_new_list_button():
    INSPECTION_LIST_MANAGEMENT_PAGE.click_new_list_button()
    sleep(3)

@then('the user should be navigated to the "NEW INSPECTION LIST" and the user should see the "List Management" child module getting highlighted Vehicle Inspection List page')
def verify_navigated_to_new_list_page():
    assert INSPECTION_LIST_MANAGEMENT_PAGE.get_trailer_new_inspection_list_title() == "NEW INSPECTION LIST"
    assert INSPECTION_LIST_MANAGEMENT_PAGE.list_management_highlighted() is True


# LQ-12422
@when(
    'the user enters required fields: "List Name", "Inspection Item Section" and "Inspection Points" & the user clicks Create')
...
@then('the trailer inspection list is updated')
...
# LQ-204429
@when('the user clicks on the "New List" button')
def click_trailer_new_list_button():
    INSPECTION_LIST_MANAGEMENT_PAGE.click_trailer_new_list_button()


@then('the user should be navigated to the "NEW INSPECTION LIST" and the user should see the "List Management" child module getting highlighted')
def verify_navigated_to_trailer_new_list_page():
    assert INSPECTION_LIST_MANAGEMENT_PAGE.get_trailer_new_inspection_list_title() == "NEW INSPECTION LIST"
    assert INSPECTION_LIST_MANAGEMENT_PAGE.list_management_highlighted() is True
...
def set_search_trailer_vehicle_name(context):
    INSPECTION_LIST_ASSIGNMENT_PAGE.click_trailer_reset_button()
    context['TRAILER_NAME'] = INSPECTION_LIST_ASSIGNMENT_PAGE.get_first_trailer_name_stg()
    INSPECTION_LIST_ASSIGNMENT_PAGE.search_trailer_name_filter(context['TRAILER_NAME'])
    sleep(3)


@then('the trailers names have inputted characters are shown')
def verify_trailer_search_vehicle(context):
    assert INSPECTION_LIST_ASSIGNMENT_PAGE.get_first_trailer_name_stg() == context['TRAILER_NAME']
...
@when('the user enters some characters into "Search Trailer Name" field & the user clicks the search icon')
def set_search_trailer_name_filter(context):
    context['TRAILER_NAME'] = INSPECTION_SCHEDULES_PAGE.get_first_trailer_name_text_stg()
    INSPECTION_LIST_ASSIGNMENT_PAGE.search_trailer_name_filter(context['TRAILER_NAME'])
    INSPECTION_SCHEDULES_PAGE.click_trailer_search_name_icon()
    sleep(3)


@then('the trailer schedule reports belong to the filtered trailers are shown')
def verify_search_trailer_name_filter(context):
    assert_that(INSPECTION_SCHEDULES_PAGE.get_first_trailer_name_text_stg(), contains_string(context['TRAILER_NAME']))
    sleep(3)
...
# LQ-169760
@given('support/driver user logs in dvir page')
def launch_browser(browser):
