...
from steps.common import FLEET_URL, ENV, AutomationDataManager, NEW_UI_FTM_URL
...
scenarios('../features/fleet_map_new_ui_shell.feature')
...
# LQ-193504
@when('the user clicks on the left faced arrow present in the left navigation panel')
def click_left_faced_arrow():
    FLEET_MAP_PAGE.click_left_faced_arrow()


@then('the user able to see the left navigation getting collapsed')
def assert_left_navigation_collapsed():
    assert FLEET_MAP_PAGE.get_left_navigation_collapsed() is True


@when('the user clicks on the right side faced arrow present in the left navigation panel')
def click_right_faced_arrow():
    FLEET_MAP_PAGE.click_right_faced_arrow()


@then('the user should be able to see the left navigation getting expanded')
def assert_left_navigation_expanded():
    assert FLEET_MAP_PAGE.get_left_navigation_expanded() is True
...
def assert_moving_track_point_detail():
    ...
    FLEET_MAP_PAGE.click_moving_panel_close_button()
