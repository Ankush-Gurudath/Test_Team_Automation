...
scenarios('../features/admin_vehicle_new_ui_shell.feature')
...
def pm_log_in(browser):
    ...
    LOGIN_PAGE.retry_if_login_failed_new_ui(DC_URL, AD.pm_only_user_name, AD.password, AD.company_name)
...
def verify_add_vehicle():
    ...
    assert_that(VEHICLE_MANAGEMENT_PAGE.get_expected_vehicle_name_new_ui(tacm.random_name), contains_string(tacm.random_name))
...
def verify_search_vehicle_filter():
    ...
    assert_that(VEHICLE_MANAGEMENT_PAGE.get_first_vehicle_name_new_ui(), contains_string(tacm.random_name))
    VEHICLE_MANAGEMENT_PAGE.click_reset_button()
...
def verify_edit_vehicle():
    assert VEHICLE_MANAGEMENT_PAGE.get_expected_vehicle_name_new_ui(
        tacm.random_name + "edited") == tacm.random_name + "edited"
...
def verify_move_group_vehicle():
    ...
    expected_vehicle = tacm.random_name + "edited"
    assert VEHICLE_MANAGEMENT_PAGE.get_first_row_group_name_new_ui(AD.group, expected_vehicle) == AD.group
    VEHICLE_MANAGEMENT_PAGE.click_reset_button()
...
def verify_change_status_vehicle():
    ...
    VEHICLE_MANAGEMENT_PAGE.click_vehicle_select_search_filter()
    VEHICLE_MANAGEMENT_PAGE.search_name_filter(tacm.random_name + "edited")
    edited_vehicle_name = tacm.random_name + "edited"
    assert VEHICLE_MANAGEMENT_PAGE.get_first_row_expected_status_name_new_ui("Out Of Service",
                                                                             edited_vehicle_name) == "Out Of Service"
...
def delete_vehicle_in_edit_vehicle_page():
    ...
    EDIT_VEHICLE_PAGE.wait_for_page_to_fully_load()
    VEHICLE_MANAGEMENT_PAGE.click_reset_button()
...
def verify_date_range_filter():
    assert_that(VEHICLE_MANAGEMENT_PAGE.get_action_date(), contains_string(VEHICLE_MANAGEMENT_PAGE.get_time_today()))
...
@given('user moved a vehicle to another group in Vehicle Management page')
def user_moved_vehicle_to_another_group(context):
    context['vehicle_name'] = VEHICLE_MANAGEMENT_PAGE.get_first_vehicle_name()
    group_name = VEHICLE_MANAGEMENT_PAGE.get_first_row_group_name()
    ...
    VEHICLE_MANAGEMENT_PAGE.click_edit_vehicle_button()
    VEHICLE_MANAGEMENT_PAGE.click_filter_edit_vehicle_button()
    VEHICLE_MANAGEMENT_PAGE.search_edit_bulk_group(group_name)
    VEHICLE_MANAGEMENT_PAGE.click_edit_bulk_select_group_button()
    VEHICLE_MANAGEMENT_PAGE.click_edit_bulk_group_done_button()
    VEHICLE_MANAGEMENT_PAGE.click_edit_bulk_apply_button()
    VEHICLE_MANAGEMENT_PAGE.click_move_group_continue()
...
# LQ-249616
@when('user clicks on "View Change History" in Vehicle Management page')
def user_clicks_view_change_history_vehicle(context):
    VEHICLE_MANAGEMENT_PAGE.click_change_view_history()
    VEHICLE_MANAGEMENT_PAGE.search_vehicle_affected(context['vehicle_name'])


@then('the Vehicle Audit Log should display logs with the Action type as "Edited" and Action details as Group')
def verify_action_type_edited_and_action_details_group():
    assert VEHICLE_MANAGEMENT_PAGE.get_first_action_type_text() == "Edited"
    assert VEHICLE_MANAGEMENT_PAGE.get_first_action_details_text() == "Group"
