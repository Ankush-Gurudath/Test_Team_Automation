...
from steps.common import DC_URL, ENV, AutomationDataManager
...
def verify_login():
    ...
    LIBRARY_PAGE.delete_recognitions()
...
def verify_add_recognition():
    DASHBOARD_PAGE.click_library_menu()
    DASHBOARD_PAGE.click_recognition_history_submenu_new_UI()
    assert RECOGNITION_HISTORY_PAGE.get_row_count() == 1

    # delete the added recognition for the next or other tests run
    LIBRARY_PAGE.delete_recognitions()
...
def verify_group_filter():
    assert DASHBOARD_PAGE.get_group_widget_1st_group() == HD.group
    assert DASHBOARD_PAGE.get_coach_widget_1st_coach() is True
    # if there is no data in drivers widget, assert the no data msg instead
    try:
        assert DASHBOARD_PAGE.get_driver_widget_1st_driver() is True
    except TimeoutException:
        assert DASHBOARD_PAGE.no_driver_data_displayed() is True
...
def verify_date_filter():
    ...
    # assert DASHBOARD_PAGE.get_driver_name_filtered() is True
    DASHBOARD_PAGE.click_reset_button()
...
def navigate_to_coaching_history_tab():
    DRIVER_PROFILE_PAGE.click_coaching_history()
