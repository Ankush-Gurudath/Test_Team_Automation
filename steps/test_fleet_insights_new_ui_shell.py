...
from steps.common import FLEET_URL, ENV, AutomationDataManager, TestDataEnum, NEW_UI_FTM_URL
...
scenarios('../features/fleet_insights_new_ui_shell.feature')
...
@when('the user clicks the dropdown of "insights" menu')
def click_insights_dropdown():
    FLEET_INSIGHT_PAGE.click_insights_new_ui()


# LQ-544
@then('the "insights" menu should expand to show the "Fleet Operations" ,"Fleet Data " , "Equipment Status" ,"Geofences " ,"State Mileage " and "Data Export"')
def assert_insights_menu_items():
    assert FLEET_INSIGHT_PAGE.get_fleet_operations_new_ui_text() == "Fleet Operations"
    assert FLEET_INSIGHT_PAGE.get_fleet_data_new_ui_text() == "Fleet Data"
    assert FLEET_INSIGHT_PAGE.get_equipment_status_new_ui_text() == "Equipment Status"
    assert FLEET_INSIGHT_PAGE.get_geofences_new_ui_text() == "Geofences"
    assert FLEET_INSIGHT_PAGE.get_state_mileage_new_ui_text() == "State Mileage"
    assert FLEET_INSIGHT_PAGE.get_data_export_new_ui_text() == "Data Export"
...
def assert_fleet_operations_date_filter_drivers_tab():
    assert FLEET_INSIGHT_PAGE.get_row_count(20) == FID.fleet_operation_drivers_tab_date_filter_count
...
def assert_fleet_operations_group_filter_vehicles_tab():
    assert FLEET_INSIGHT_PAGE.get_row_count(20) == FID.fleet_operation_vehicles_tab_group_filter_count
...
def assert_fleet_operations_date_filter_vehicles_tab():
    assert FLEET_INSIGHT_PAGE.get_row_count(20) == FID.fleet_operation_vehicles_tab_date_filter_count
...
def assert_vehicle_profile_summary():
    ...
    assert FLEET_INSIGHT_PAGE.get_vehicle_profile_idle_violation() == "IDLE OVERAGE"
    assert FLEET_INSIGHT_PAGE.get_vehicle_profile_idle_duration() == "IDLE VIOLATIONS DURATION"
    assert FLEET_INSIGHT_PAGE.get_vehicle_profile_speed_violation() == "SPEED VIOLATIONS"
    assert FLEET_INSIGHT_PAGE.get_vehicle_profile_speed_duration() == "SPEED VIOLATIONS DURATION"
...
def assert_vehicle_profile_idle_detail():
    ...
    assert FLEET_INSIGHT_PAGE.get_idle_duration_vehicle_profile() == 'IDLE VIOLATIONS DURATION'
...
def assert_state_mileage():
    ...
    assert FLEET_INSIGHT_PAGE.count_of_records_state_mileage_is_displayed() is True
...
def assert_filter_group_state_mileage():
    assert FLEET_INSIGHT_PAGE.get_row_count(20) == FID.state_mileage_group_filter_count
...
def assert_filter_date_state_mileage():
    assert FLEET_INSIGHT_PAGE.get_row_count(20) == FID.state_mileage_date_filter_count
