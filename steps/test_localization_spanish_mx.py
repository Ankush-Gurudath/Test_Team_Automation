...
def verify_login():
    ...
    assert_that(DASHBOARD_PAGE.get_tasks_title(), IsEqualIgnoringCase(LDI.tasks_es_mx_label))
    assert_that(DASHBOARD_PAGE.get_metrics_title(), IsEqualIgnoringCase(LDI.metrics_es_mx_label))
...
def verify_coaching_session_page():
    assert EVENT_PAGE.event_date_month_is_word() is False
