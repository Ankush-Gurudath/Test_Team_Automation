...
scenarios('../features/pod2_prod_sanity.feature')
...
def login():
    ...
    LOGIN_PAGE.retry_if_login_failed_new_ui(WELCOME_URL, HD.pod2_full_access_username, HD.password2)
...
def click_browse_button(context):
    if VIDEO_SEARCH_PAGE.browse_button_is_displayed() is False: # no video tags present from previous test case
        VIDEO_SEARCH_PAGE.click_vehicle_tab_new()
...
def login_risk_company():
    LOGIN_PAGE.enter_username(RCD.risk_fa_user_pod2)
    LOGIN_PAGE.enter_password(RCD.password)
    LOGIN_PAGE.click_login()
    LOGIN_PAGE.retry_if_login_failed_new_ui(DC_URL, RCD.risk_fa_user_pod2, RCD.password)
