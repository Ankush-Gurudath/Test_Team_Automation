...
scenarios('../features/admin_device.feature')
...
def verify_group_filter_device_management():
    assert_that(DEVICE_MANAGEMENT_PAGE.get_first_group_name(AD.devicegroup_group),
                IsEqualIgnoringCase(AD.devicegroup_group))


@when('the user selects multiple groups from the "Select Group(s)" filter')
def apply_group_filter_device_management():
    DEVICE_MANAGEMENT_PAGE.click_reset_button()
    DEVICE_MANAGEMENT_PAGE.click_group_filter()
    DEVICE_MANAGEMENT_PAGE.search_group_filter(AD.Automation_group1)
    DEVICE_MANAGEMENT_PAGE.select_searched_group()
    DEVICE_MANAGEMENT_PAGE.click_done_group_filter()


@then('the device belong to the selected groups are listed')
def verify_group_filter_device_management():
    assert DEVICE_MANAGEMENT_PAGE.get_device_count() == '7'
    for i in range(1, 8):
        assert_that(DEVICE_MANAGEMENT_PAGE.get_devices_groups_label(i),contains_string(getattr(AD, f"Automation_group{i}")))
...
# LQ-249538
@given('user clicks on "View Change History" in Device Management page')
def user_clicks_view_change_history_device_management():
    DEVICE_MANAGEMENT_PAGE.click_view_change_history()
