...
def verify_login():
    ...
    assert_that(DASHBOARD_PAGE.get_tasks_new_UI_title(), IsEqualIgnoringCase(LDI.tasks_es_label))
    assert_that(DASHBOARD_PAGE.get_metrics_new_UI_title(), IsEqualIgnoringCase(LDI.metrics_es_label))
...
def verify_overlay_label():
    assert_that(EVENT_PAGE.get_overlay_label_text(), IsEqualIgnoringCase(LDI.overlay_es_label))
...
def verify_overlay_label():
    assert_that(EVENT_PAGE.get_overlay_toggle_text(), IsEqualIgnoringCase('La superposición no está disponible para este vídeo.'))
...
def verify_file_name_format():
    expected_file_name = f"{LDI.fd_event_with_adas_data}-{LDI.overlay_es_label}.MP4"
