...
def run_pytest_bdd(config, feature, output, task_id=0):
    if platform.system() == 'Windows':
        sh('SET CONFIG_FILE=config/%s.json & SET TASK_ID=%s & pytest %s --cucumberjson %s -v -s' % (
            config, task_id, feature, output))
    else:
        sh('export CONFIG_FILE=config/%s.json && export TASK_ID=%s && pytest %s --cucumberjson %s -v -s' % (
            config, task_id, feature, output))
