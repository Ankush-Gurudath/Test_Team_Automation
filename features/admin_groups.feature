Feature: Admin - Group Management
  ...
  Scenario: Verify search functionality with invalid group name
    ...
    Then drop down should not present
  ...
  Scenario: Verify that the Group Audit log tab is displayed
    ...
    And The user should be able to see the filters ‘Search Group Affected’, Date filter and ‘Select Action(s)’ in the header, and the ‘Download Log’ button
